...
- Validate every task by running `esphome config` against `packages/orbital_base.yaml` (install via `pip install esphome` first if not already installed) — this is this repo's equivalent of running tests; there is no other test runner.
- **Correction found during Task 1 implementation, verified against the official ESPHome `packages.mdx` docs and reproduced on this repo's own existing `wopr_base.yaml`/`s31_base.yaml`:** `api:` requires a network (`wifi:`/`ethernet:`) component to be present in the *same resolved config* to pass `esphome config`, so `esphome config packages/orbital_base.yaml` can never pass standalone once the package defines `api:` — this is true of every package in this repo, not a defect in this one. Per the official docs' own example (a `device_base.yaml` with `api:` but no `wifi:`, paired with a separate `wifi.yaml` merged in only at the device level), the package itself must **not** define `wifi:`/`ota:`/credentials — that stays a device-layer concern, exactly like `wopr_base.yaml` already does. To validate the package while it's under construction (Tasks 1–9), wrap it in an uncommitted scratch file (e.g. `/tmp/orbital_dev_check.yaml`) that supplies a throwaway `wifi:`/`ota:` around `packages: base: !include <path-to>/packages/orbital_base.yaml`, run `esphome config` against *that* scratch file, and discard it — never commit it, never add real `wifi:`/`ota:` to `packages/orbital_base.yaml` itself. Task 10 is where a real, permanent test wrapper (`tests/test_orbital.yaml`) gets created.
